...
class ProcessList:
    ...
    def add_ready_process(self, process):  # 在就绪队列中添加进程
        process.psw = 1  # 设置进程状态为就绪态
        process.WaitTimes = clock.Clock.current - process.InTimes  # 计算等待时间
        self.ready_processes.append(process)

    def add_running_process(self, index=None):  # 在运行队列中添加进程
    ...
    def add_running_process_no_time_slice(self):  # 在运行队列中添加进程
        ...
        if len(self.ready_processes) == 0:
            ...
            while self.running_processes.InstrucNum > self.running_processes.ir:
                window.ui.printf(
                    f"{clock.Clock.current}:[运行进程:{self.running_processes.ProID},{self.running_processes.data[self.running_processes.ir][0]},计算]")
                self.running_processes.ir += 1
                self.running_processes.pc = self.running_processes.ir - 1
    ...
    # def add_running_process_index(self, index=None):  # 在运行队列中添加进程
    #     if len(self.ready_processes) == 0:
    #         return None
    #     if index is None:
    #         index = 0
    #
    #     # 在运行队列的第一位添加进程
    #     process = self.ready_processes.pop(index)
    #     self.running_processes = process
    #
    #     CPU.cpu.ir = self.running_processes.ProID  # 设置CPU的ir
    #     while self.running_processes.InstrucNum > self.running_processes.ir:
    #         window.ui.printf(
    #             f"{clock.Clock.current}:[运行进程:{self.running_processes.ProID},{self.running_processes.data[self.running_processes.ir][0]},计算]")
    #         self.running_processes.ir += 1
    ...
    def run_time_slice(self, ptime):  # 时间片轮转
        ...
        if self.running_processes.psw == -1:
            ...
            self.running_processes.TurnTimes.append([st, et])  # 记录进程的运行时间
            window.ui.printf(
                f"{clock.Clock.current}:[重新进入就绪队列:{self.running_processes.ProID},{self.running_processes.InstrucNum}]")
