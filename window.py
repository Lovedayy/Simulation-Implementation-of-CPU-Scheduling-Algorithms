    ...
    def slotAdd(self):  # 向界面右侧添加数据
        ...
        try:  # 无进程运行，则显示0
            ...
            self.textEdit_5.setText(
                str(ProcessList.pro.running_processes.InstrucNum - ProcessList.pro.running_processes.ir))  # 添加剩余指令数
        ...
        if ProcessList.idea == 1 or 3 or 4 or 5:
            self.table_add()
    ...
    def table_add(self):  # 向就绪队列表格中添加数据
        ...
        for i in range(len(ProcessList.pro.ready_processes)):
            ...
            item = QTableWidgetItem(
                str(ProcessList.pro.ready_processes[i].InstrucNum - ProcessList.pro.ready_processes[i].ir))  # 显示剩余指令数
