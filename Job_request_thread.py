...
class Job_request_thread():
    ...
    # 不断读取作业请求，并将其放入就绪队列中，直到所有作业请求都被处理完毕
    def run(self):
        ...
        if len(ProcessList.pro.ready_processes) < 10:
            ...
            if self.PCB_list[self.i].InTimes <= clock.Clock.current:
                if self.PCB_list[self.i].psw == 0:  # 判断是否是新建进程
                    ...
                    window.ui.printf(
                        f"{t}:[新增作业请求:{self.PCB_list[i].ProID},{self.PCB_list[i].InTimes},{self.PCB_list[i].InstrucNum}]")
                    window.ui.printf(f"{t}:[创建进程:{self.PCB_list[self.i].ProID}]")
                    window.ui.printf(
                        f"{t}:[进入就绪队列:{self.PCB_list[self.i].ProID},{self.PCB_list[self.i].InstrucNum}]")
                    ProcessList.pro.add_ready_process(self.PCB_list[self.i])
                if self.i < len(self.PCB_list) - 1:  # 判断是否还有作业请求
                    self.i += 1

    # 将所有作业请求的状态信息（包括进入时间、运行时间、结束时间等）输出到窗口
    def Put_PCB_Data(self):  # 输出结果
        ...
        for i in range(len(self.PCB_list)):
            window.ui.printf(
                f"{self.PCB_list[i].EndTimes}:[{self.PCB_list[i].ProID}:{self.PCB_list[i].InTimes}+{self.PCB_list[i].RunTimes}+{self.PCB_list[i].EndTimes - self.PCB_list[i].InTimes}]")
